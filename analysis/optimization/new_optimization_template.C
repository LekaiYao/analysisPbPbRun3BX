...
void optimization_@VAR@() {
    //TFile *file_data = TFile::Open("../TMVA/sideband_PSI_BDT_v3test1.root");
    TFile *file_data = TFile::Open("../selection/root_files/sideband.root");
    TTree *tree_data = (TTree*)file_data->Get("tree");

    // --- MC for X(3872) (original) ---
    TFile *file_MC = TFile::Open("../selection/root_files/MC_X3872.root");
    TTree *tree_MC = (TTree*)file_MC->Get("tree");

    // --- MC for PSI(2S) (added) ---
    TFile *file_MC_psi = TFile::Open("../selection/root_files/MC_PSI2S.root");
