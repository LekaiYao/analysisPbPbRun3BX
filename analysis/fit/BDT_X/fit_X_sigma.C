...
void fit_X_sigma() {
	...
	RooFitResult* fit_sb=tot.fitTo(*newdata,Save(),Minimizer("Minuit2", "Migrad"));

	//draw the fitX/Y and the leg
	TCanvas *c_Bmass=new TCanvas("c_Bmass","c_Bmass",10,10,800,600);
	RooPlot *frame_Bmass=Bmass.frame(RooFit::Title("Fit X"),Bins(BinNum));
	...
	c_Bmass->SaveAs("./pdf_X_sigma/Xmass_test4_sb_v1.pdf");
	...
	double Nll_sb = fit_sb->minNll(), Nll_b = fit_b->minNll();
