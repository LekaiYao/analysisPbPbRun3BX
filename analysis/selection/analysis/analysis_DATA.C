...
//      root> .L analysis_DATA.C
//      root> analysis_DATA t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   if (fChain == 0) return;
   TFile* outputFile = new TFile("DATA_XPSI.root", "RECREATE");
   ...
   //define the branches
   tree->Branch("B_type", &B_type, "B_type/I");
   ...
   tree->Branch("B_gen", &B_gen, "B_gen/F");
   tree->Branch("B_genIndex", &B_genIndex, "B_genIndex/I");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      // Lightweight progress indicator every ~5%
        if (( jentry % (nentries / 20 + 1)) == 0) {
            printf("Processed %lld / %lld (%.1f%%)\r",
                   jentry, nentries, 100.0 * double(jentry) / double(nentries));
            fflush(stdout);
        }
      if(Bsize == 0) continue;
      Long64_t i_B = 0;
