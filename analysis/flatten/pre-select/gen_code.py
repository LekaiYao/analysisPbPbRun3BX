...
def main(filename):
    ...
    # struct definition (no leading space before 'struct Out {')
    print("struct Out {")
    for t, n, arr in vars:
        print(f"\t{t} {n};")   # one tab before each line
    print("\t};")  # one tab before the closing brace
    print("\nOut out;")

    # Branch definitions block
    print("\n// Branch definitions")
    print("{")
    for t, n, arr in vars:
