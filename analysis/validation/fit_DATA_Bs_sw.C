...
void fit_DATA_Bs_sw() {
    TFile *f = TFile::Open("../selection/root_files/Bs/DATA_Bs_cut1.root");
    TTree *tree = (TTree*)f->Get("tree");
    
    float lowBmass=5.0,highBmass=5.8;
	...
	//signal of Bs
	RooRealVar mean_Bs("mean_Bs","mean_Bs",5.37,5.34,5.40);
	RooRealVar sigma1_Bs("sigma1_Bs","sigma1_Bs",0.02,1e-03,0.03);
	RooGaussian Gaus1_Bs("Gaus1_Bs","Gaus1_Bs",Bmass,mean_Bs,sigma1_Bs);
	RooRealVar sigma2_Bs("sigma2_Bs","sigma2_Bs",0.005,1e-04,0.01);
	RooGaussian Gaus2_Bs("Gaus2_Bs","Gaus2_Bs",Bmass,mean_Bs,sigma2_Bs);
	RooRealVar frac_Bs("frac_Bs","frac_Bs",0.5,0.2,0.8);
	RooAddPdf sig_Bs("sig_Bs","sig_Bs",RooArgList(Gaus1_Bs,Gaus2_Bs),frac_Bs);
	...
	RooRealVar n_sig_Bs = RooRealVar("n_sig_Bs","n_sig_Bs",2000,0,N);
	RooRealVar n_bkg = RooRealVar("n_bkg","n_bkg",8000,0,N);

	RooExtendPdf sige_Bs("sige_Bs","sige_Bs",sig_Bs,n_sig_Bs);
	RooExtendPdf bkge("bkge","bkge",bkg,n_bkg);
	
	//add them together
	//RooAddPdf tot("total","total",RooArgList(sige_psi2s,sige_X,bkge),RooArgList(n_sig_psi2s,n_sig_Bs,n_bkg));
	RooAddPdf tot("total","total",RooArgList(sige_Bs,bkge),RooArgList(n_sig_Bs,n_bkg));
    ...
    SPlot* sData = new SPlot("sData", "sWeights for Bs",
                              data, &tot,
                              RooArgList(n_sig_Bs, n_bkg));

    cout << "sWeights computed." << endl;
    cout << "Total entries in data: " << data.numEntries() << endl;
    cout << "n_sig_Bs from sPlot: " << sData->GetYieldFromSWeight("n_sig_Bs") << endl;
    cout << "n_bkg from sPlot: " << sData->GetYieldFromSWeight("n_bkg") << endl;

    // -------------------- Save new ROOT file with sWeights --------------------
    TFile *fout = new TFile("./sw_root/Bs/DATA_Bs_cut1_sw.root","RECREATE");
    TTree* outtree = tree->CloneTree(0);
	outtree->SetName("tree");
	outtree->SetTitle("Tree with sWeights for Bs");

    double w_Bs;
    outtree->Branch("sWeight",&w_Bs,"sWeight/D");

    for (int i=0;i<data.numEntries();i++){
		tree->GetEntry(i);          
    	w_Bs = sData->GetSWeight(i,"n_sig_Bs"); 
    	outtree->Fill();
    }
	...
	tot.plotOn(frame_Bmass, Components(sig_Bs),     LineColor(pink),  LineStyle(2), Name("sig_Bs"));
	...
	leg1.AddEntry(frame_Bmass->findObject("sig_Bs"),      "signal Bs",     "L");
	...
	c_Bmass->SaveAs("./pdf/Bs/Bs_DATA_sw_v1.pdf");
