...
void fit_DATA_XPSI_sw() {
    TFile *f = TFile::Open("../selection/root_files/X/DATA_X_cut1.root");
	...
	RooGaussian Gaus1_psi2s("Gaus1_psi2s","Gaus1_psi2s",Bmass,mean_psi2s,sigma1_psi2s);
	RooRealVar sigma2_psi2s("sigma2_psi2s","sigma2_psi2s",0.005,1e-04,0.006);
	RooGaussian Gaus2_psi2s("Gaus2_psi2s","Gaus2_psi2s",Bmass,mean_psi2s,sigma2_psi2s);
	...
	RooGaussian sig_X("sig_X","sig_X",Bmass,mean_X,sigma_X);
	...
	RooChebychev bkg("bkg", "Chebychev background", Bmass, RooArgList(c1, c2, c3));
	...
	RooRealVar n_bkg = RooRealVar("n_bkg","n_bkg",300000,0,N);
	...
	m.setEps(5000.0); //edm < 0.001*"setEps"; edm < 1 is OK for more than 100k events
	m.setPrintLevel(1); 
    ...
    SPlot* sData = new SPlot("sData", "sWeights for Psi(2S) and X(3872)",
                              data, &tot,
                              RooArgList(n_sig_psi2s, n_sig_X, n_bkg));

    cout << "sWeights computed." << endl;
    cout << "Total entries in data: " << data.numEntries() << endl;
    cout << "n_sig_psi2s from sPlot: " << sData->GetYieldFromSWeight("n_sig_psi2s") << endl;
    cout << "n_sig_X from sPlot: " << sData->GetYieldFromSWeight("n_sig_X") << endl;
    cout << "n_bkg from sPlot: " << sData->GetYieldFromSWeight("n_bkg") << endl;

    // -------------------- Save new ROOT file with sWeights --------------------
    TFile *fout = new TFile("./sw_root/X/DATA_X_cut0_sw.root","RECREATE");
    TTree* outtree = tree->CloneTree(0);
	outtree->SetName("tree");
	outtree->SetTitle("Tree with sWeights for X(3872)");

    double w_X;
    outtree->Branch("sWeight_X",&w_X,"sWeight_X/D");

    for (int i=0;i<data.numEntries();i++){
		tree->GetEntry(i);          
    	w_X = sData->GetSWeight(i,"n_sig_X"); 
    	outtree->Fill();
    }
    ...
    cout << "\n>>> Saved new ROOT file with sWeights: ./sw_root/X/DATA_X_cut0_sw.root\n" << endl;
    ...
    RooPlot* frame_zoom = Bmass.frame(Range(zoom_low, zoom_high), Bins(zoom_bins));
    frame_zoom->SetTitle("");
    ...
    if (draw_box_on_main) {
		...
		TH1* htemp = data.createHistogram("htemp", Bmass,
	...
	c_Bmass->SaveAs("./pdf/X/PsiX_DATA_sw_v1.pdf");
